...
public:
  enum ColorIndex { PURPLE, GREEN, BLUE, UNDEFINED };

  static inline std::string IdxToStr(ColorIndex ind) { return colorNames[ind]; }

  static ColorIndex detectColorIndex(const cv::Scalar &a) {
    // pair<ColorIndex, double>  |  ColorIndex, color distance between a and
    // Colors[ColorIndex]
    auto cmp = [](const std::pair<ColorIndex, double> &a,
                  const std::pair<ColorIndex, double> &b) {
      return a.second < b.second;
    };

    std::set<std::pair<ColorIndex, double>, decltype(cmp)> colorIndexSet;

    colorIndexSet.insert(std::pair<ColorIndex, double>(
        ColorIndex::PURPLE, ColorDistance(colors[ColorIndex::PURPLE], a)));
    colorIndexSet.insert(std::pair<ColorIndex, double>(
        ColorIndex::GREEN, ColorDistance(colors[ColorIndex::GREEN], a)));
    colorIndexSet.insert(std::pair<ColorIndex, double>(
        ColorIndex::BLUE, ColorDistance(colors[ColorIndex::BLUE], a)));
...
const cv::Rect PROP_RECTS[] = {{314, 167, SQUARE_SIZE_X, SQUARE_SIZE_Y},
...
std::vector<
    std::array<std::tuple<ColorDetector::ColorIndex, std::string, int>, 9>>
    DESIRED_RESULT;

/*
    std::make_tuple(ColorDetector::ColorIndex::UNDEFINED, "", 0),             //
   0 upper left corner std::make_tuple(ColorDetector::ColorIndex::UNDEFINED, "",
   0),             // 1 upper middle
    std::make_tuple(ColorDetector::ColorIndex::UNDEFINED, "", 0),             //
   2 upper right corner std::make_tuple(ColorDetector::ColorIndex::UNDEFINED,
   "", 0),             // 3 middle left
    std::make_tuple(ColorDetector::ColorIndex::PURPLE, "stun resistance", 4), //
   4 central stun resistance
    std::make_tuple(ColorDetector::ColorIndex::UNDEFINED, "", 0),             //
   5 middle right std::make_tuple(ColorDetector::ColorIndex::UNDEFINED, "", 0),
   // 6 lower left corner std::make_tuple(ColorDetector::ColorIndex::UNDEFINED,
   "", 0),             // 7 lower middle corner
    std::make_tuple(ColorDetector::ColorIndex::UNDEFINED, "", 0)              //
...
bool getDesiredResultFromJson(
  ...
  if (charName == SettingsTree.not_found() or
      expertisParametrs == SettingsTree.not_found())
    return false;

  r_charName = charName->second.get_value<std::string>();

  const std::vector<std::string> corners = {"UpLeft",  "UpMid",  "UpRight",
                                            "MidLeft", "MidMid", "MidRight",
                                            "DwnLeft", "DwnMid", "DwnRight"};

  auto expertisParametrsIt = expertisParametrs->second.begin();

  while (expertisParametrsIt != expertisParametrs->second.end()) {
    std::array<std::tuple<ColorDetector::ColorIndex, std::string, int>, 9>
        data = {std::make_tuple(ColorDetector::ColorIndex::UNDEFINED, "", 0),
                std::make_tuple(ColorDetector::ColorIndex::UNDEFINED, "", 0),
                std::make_tuple(ColorDetector::ColorIndex::UNDEFINED, "", 0),
                std::make_tuple(ColorDetector::ColorIndex::UNDEFINED, "", 0),
                std::make_tuple(ColorDetector::ColorIndex::UNDEFINED, "", 0),
    ...
    for (int i = 0; i < 9; i++) {
      ...
      if (color == itCorner->second.not_found() or
          propName == itCorner->second.not_found() or
          val == itCorner->second.not_found())
        return false;

      std::get<0>(data[i]) = static_cast<ColorDetector::ColorIndex>(
          color->second.get_value<int>());
...
int main() {
  ...
  bool jsonSucc = getDesiredResultFromJson("settings_ExpertiseRoller.json",
                                           DESIRED_RESULT, charName_u8);
  ...
  while (1) {
    ...
    if (screen.empty() or screen.cols != 1600 or screen.rows != 900) {
      Beep(2000, 500);
      std::cerr << "Failed to capture image or image size is not 1600x900!"
                << std::endl;
      continue;
    } else {
      ...
      for (int i = 0; i < DESIRED_RESULT.size(); i++) {
        ...
        for (int j = 0; j < 9; j++) {
          ...
          if ((desiredColorIdx != ColorDetector::ColorIndex::UNDEFINED &&
              ...
              (!desiredPropName.empty() &&
               (desiredPropName != Props[j].propName or
                desiredPropVal > Props[j].propVal))) {
            requiredRoll = true;
            break;
          }
        }
