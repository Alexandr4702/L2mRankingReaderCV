...
const std::array<cv::Scalar, 3> ColorDetector::colors = {cv::Scalar(80.1043, 18.2386, 38.3279, 0),
                                                         cv::Scalar(34.7525, 54.0646, 12.7087, 0),
                                                         cv::Scalar(84.3319, 36.4075, 16.8168, 0)};

const std::array<std::string, 4> ColorDetector::colorNames = {"PURPLE", "GREEN", "BLUE", "UNDEFINED"};
...
const uint16_t SQUARE_SIZE_X = 198, SQUARE_SIZE_Y = 119;
const cv::Rect PROP_RECTS[] = {{314, 198, SQUARE_SIZE_X, SQUARE_SIZE_Y}, {534, 198, SQUARE_SIZE_X, SQUARE_SIZE_Y},
                               {754, 198, SQUARE_SIZE_X, SQUARE_SIZE_Y},

                               {314, 340, SQUARE_SIZE_X, SQUARE_SIZE_Y}, {534, 340, SQUARE_SIZE_X, SQUARE_SIZE_Y},
                               {754, 340, SQUARE_SIZE_X, SQUARE_SIZE_Y},
                              ...
                              std::string &r_charName)
{
    ...
    const std::vector<std::string> corners = {"UpLeft",   "UpMid",   "UpRight", "MidLeft", "MidMid",
                                              "MidRight", "DwnLeft", "DwnMid",  "DwnRight"};
    ...
    while (expertisParametrsIt != expertisParametrs->second.end())
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            ...
            if (color == itCorner->second.not_found() or propName == itCorner->second.not_found() or
    ...
    while (1)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                ...
                // std::string win_name = std::string("exp_") + to_string(i);
                // imshow(win_name, propSquare);
                ...
                for (int j = 0; j < 9; j++)
                {
                    ...
                    if ((desiredColorIdx != ColorDetector::ColorIndex::UNDEFINED &&
                        ...
                        (!desiredPropName.empty() &&
                         (desiredPropName != Props[j].propName or desiredPropVal > Props[j].propVal)))
                    {
                        requiredRoll = true;
                        break;
                    }
